...
if [[ \$(supervisorctl status nezha) =~ STOPPED ]]; then
  ...
  if [[ "\$LATEST" =~ ^v([0-9]{1,3}\.){2}[0-9]{1,3}\$ && "\$NOW" != "\$LATEST" ]]; then
    ...
    wget -O /dashboard/app https://raw.githubusercontent.com/fscarmen2/Argo-Nezha-Service-Container/main/app/app-\$(arch) && chmod +x /dashboard/app
    echo "\$LATEST" > /version
  fi
  TIME=\$(date "+%Y-%m-%d-%H:%M:%S")
  tar czvf \$GH_REPO/dashboard-\$TIME.tar.gz --exclude /dashboard/app /dashboard
